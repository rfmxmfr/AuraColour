            ...
            <div key={ index } className="flex flex-col items-center">
              <div 
                className={ `w-6 h-6 md:w-8 md:h-8 rounded-full flex items-center justify-center text-xs md:text-sm font-medium transition-all duration-300 ${
                  index < currentStep 
                    ?  'apos;bg-gradient-to-r from-purple-600 to-pink-600 text-whitee'apos; 
                    : index === currentStep
                      ?  'apos;bg-white border-2 border-purple-600 text-purple-6000'apos;
                      :  'apos;bg-gray-200 text-gray-5000'apos;
                }` }
              >
                { index < currentStep ?  'apos;âœ““'apos; : index + 1 }
              </div>
              { stepLabels && stepLabels[index] && (
                <span className={ `text-xs mt-1 md:mt-2 text-center hidden xs:block ${
                  index <= currentStep ?  'apos;text-purple-600 font-mediumm'apos; :  'apos;text-gray-5000'apos;
                }` }>
